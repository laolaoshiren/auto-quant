 ...
 * ## 为什么不复用主题里的语义色
 *
 * 主题里的 `up`（绿）/ `down`（红）/ `warn`（琥珀）在交易界面里含义是固定的
 * —— 它们表示涨跌与风险。**拿它们去"区分币种"会制造假信号**：
 * 一个红色的 BTCUSDT 看起来像"BTC 在跌"，而它只是恰好排到了红色那一档。
 *
 * 所以这里用一组**中性的、彼此可辨的**色相，全部避开红/绿/琥珀的语义区间。
 *
 * ## 为什么按名字哈希而不是按顺序分配
 *
 * 按"第几个出现"分配颜色会让同一个币种在不同表格、翻页之后变色 ——
 * 而颜色的唯一用途就是**让人一眼认出"这是同一个东西"**。
 * 哈希保证：**同一个币种永远是同一个颜色，无论出现在哪张表、哪一页。**
 *
 * 碰撞是允许的（只有 8 档）：颜色是**辅助**，名称本身才是身份。
 */
const SYMBOL_TONES = [
  '#6ea8fe', // 蓝
  '#a78bfa', // 紫
