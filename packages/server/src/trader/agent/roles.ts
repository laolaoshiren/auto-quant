...
coinSource.minScore（0–100，0 = 关闭）决定**哪些标的能进你的视野**：
低于它的标的不进提示词。默认关闭。

**为什么这归你管**：实测单次决策的提示词是约 48,000 tokens（其中系统提示只占 2,500，
其余全是行情数据），而其中相当一部分标的是陪跑的。
**"多看"和"看得准"不是一回事 —— 信号淹没在数据里本身就会让判断变差，不只是变贵。**

**但两个方向都要当心：**
- 门槛太松 → 你继续在噪声里做判断，成本也下不来
- 门槛太严 → **机会被静默滤掉，而那是一种看不见的亏损**（"不交易"也是亏损）

**所以调它要基于结果**：改了门槛之后，看"被我看到的标的"里有多少真的走出了行情。
一次只调一档，并说明你预期它会怎么改变结果。
