...
export const STRATEGY_PRESETS: StrategyPreset[] = [
  ...
  },
  {
     ...
     * 全自动智能托管。
     *
     * ⚠️ **这个预设的 patch 只提供"起点"，不是"设定"。** 选中它之后，
     * 机器人的参数会由 AI 智能体在运行过程中自己调整（存在 `traders.agent_config_json`，
     * 按机器人隔离），这里给的数值只用于**第一次启动时**。
     *
     * 所以这份 patch 的取向是"**保守的起点**"而不是"最优参数"：
     * 起点的作用是让 AI 有一个可参照的基线去改，而不是替它做决定。
     * 把起点设得激进会让它一上来就在大仓位上试错 —— 而试错的代价是真钱。
     */
    id: 'ai_managed',
    label: '全自动智能托管',
    summary: '由 AI 智能体自主设定并持续调整全部参数，以盈利为唯一目标。',
    tradingMode: 'conservative',
    /*
     * 起点照抄「稳健」的钱与风险参数，**不自己编 indicators**。
     *
     * 两个原因，都是实测撞出来的：
     *  1. 我一开始凭记忆写了一整块 indicators，字段名错了好几个
     *     （`rsiPeriod` 应为 `rsiPeriods`、`primaryCandleCount` 应为 `primaryCount`）。
     *     指标字段由 schema 决定，抄现有预设比自己编可靠。
     *  2. 已有的测试 `every preset produces a budget that fits inside the token ceiling`
     *     会校验**每个**预设的提示词预算 —— 我那个 16 项指标 + 大候选池直接超了。
     *     那条测试正好在做它该做的事。
     */
    patch: {
      tradingMode: 'conservative',
      ...
      customPrompt:
        '这是一个由 AI 自主托管的账户：参数由你在运行过程中调整，以盈利为唯一目标，但**先活下来** —— ' +
        '一次亏损不该威胁到账户的存续。入场前先想清楚这笔会在什么情况下亏、以及亏多少。' +
        '样本不足时不要下结论：少于 30 笔平仓的成绩分不清是策略有效还是运气好。',
    },
  },
];
